...
export function createProviderRoutes(requireAuth: Function, requireAdmin: Function): Router {
  ...
  /** POST /api/providers/geo/w3w — convert What3Words address to coordinates */
  router.post("/geo/w3w", requireAuth as any, async (req: Request, res: Response) => {
    const { words } = req.body;
    if (!words || typeof words !== "string" || !/^[a-z]+\.[a-z]+\.[a-z]+$/i.test(words)) {
      return res.status(400).json({ error: "words must be in format 'word.word.word'" });
    }

    const apiKey = process.env.WHAT3WORDS_API_KEY;
    if (!apiKey) {
      return res.status(503).json({ error: "W3W service not configured" });
    }

    try {
      const url = new URL("https://api.what3words.com/v3/convert-to-coordinates");
      url.searchParams.set("words", words.toLowerCase());
      url.searchParams.set("key", apiKey);

      const response = await fetch(url.toString(), { signal: AbortSignal.timeout(8000) });
      if (!response.ok) {
        const err = await response.json().catch(() => ({}));
        return res.status(502).json({ error: (err as any).error?.message || "W3W API error" });
      }
      ...
      res.status(502).json({ error: "W3W service unavailable" });
    }
  });
